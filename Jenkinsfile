node
{
    ...
    stage('Build Docker Image')
     {
    sh 'docker build -t torretto/jenkins-jobs:v5 .'
    }
    ...
    sh 'docker push torretto/jenkins-jobs:v5'
    }
    stage('Deploy Docker Container on Server')
     { 
    def dockerRun = 'sudo docker run -p 8080:8080 -d --name my-app torretto/jenkins-jobs:v5'
    sshagent(['Deployment-server']) 
    {
    sh "ssh -o StrictHostKeyChecking=no ec2-user@172.31.8.206 ${dockerRun}"
     }
    }
}
